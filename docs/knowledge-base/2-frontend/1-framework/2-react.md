...
**Hooks 就是这样一套为函数组件设计的，用户访问 React 的内部状态或执行副作用操作，以函数形式存在的 React API**。这里的“内部状态”，除了组件的 state，还包括 context、memo、ref 等。

#### 函数组件

组件的 state 并不是绑定在组件函数上的，而是绑定在组件渲染产生的虚拟 DOM 节点（FiberNode）上的。所以在函数组件中调用 useState，意味着将访问组件以外的、React 以内的状态，这就让函数产生了副作用，函数组件就不再是“纯函数组件”。但是这样一来，函数组件虽然不再纯粹，功能却更加强大了，可以使用 React 的绝大部分特性了。
